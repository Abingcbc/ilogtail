...
void HostMonitorInputRunner::UpdateCollector(const std::string& configName,
    ...
    for (size_t i = 0; i < newCollectorNames.size(); ++i) {
        ...
        auto collector = mCollectorCreatorMap[collectorName]();
        ...
        collectContext.PrepareForFirstCollect();

        { // add collector to registered collector map
            std::unique_lock<std::shared_mutex> lock(mRegisteredCollectorMapMutex);
            auto iter = mRegisteredCollectorMap.find(configName);
            if (iter == mRegisteredCollectorMap.end()) {
                mRegisteredCollectorMap.emplace(configName, std::unordered_map<std::string, CollectorInstance>());
                mRegisteredCollectorMap[configName].emplace(collectorName, std::move(collector));
            } else {
                iter->second.emplace(collectorName, std::move(collector));
            }
        }
...
void HostMonitorInputRunner::RemoveCollector(const std::string& configName) {
    ...
    mRegisteredCollectorMap.erase(iter);
}
...
bool HostMonitorInputRunner::IsCollectTaskValid(const std::chrono::steady_clock::time_point& startTime,
    ...
    std::shared_lock<std::shared_mutex> lock(mRegisteredCollectorMapMutex);
    auto it = mRegisteredCollectorMap.find(configName);
    if (it == mRegisteredCollectorMap.end()) {
        return false;
    }
    auto collector = it->second.find(collectorName);
    if (collector == it->second.end()) {
        return false;
    }
    return collector->second.IsValidEvent(startTime);
}
...
void HostMonitorInputRunner::ScheduleOnce(HostMonitorTimerEvent::CollectContext& context) {
    auto collectFn = [this, context = std::move(context)]() mutable {
        try {
            ...
            std::shared_lock<std::shared_mutex> lock(mRegisteredCollectorMapMutex);
            auto configIter = mRegisteredCollectorMap.find(context.mConfigName);
            if (configIter == mRegisteredCollectorMap.end()) {
                LOG_ERROR(sLogger,
                          ("config not found, will not collect again",
                           "discard data")("config", context.mConfigName)("collector", context.mCollectorName));
                return;
            }
            auto collectorIter = configIter->second.find(context.mCollectorName);
            if (collectorIter == configIter->second.end()) {
                LOG_ERROR(sLogger,
