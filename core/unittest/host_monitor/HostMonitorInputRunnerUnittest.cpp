...
public:
    ...
    void TestPrepareForFirstCollect() const;
...
void HostMonitorInputRunnerUnittest::TestUpdateAndRemoveCollector() const {
    ...
    APSARA_TEST_TRUE_FATAL(
        runner->IsCollectTaskValid(std::chrono::steady_clock::now(), configName, MockCollector::sName));
    APSARA_TEST_FALSE_FATAL(runner->IsCollectTaskValid(
        std::chrono::steady_clock::now() - std::chrono::seconds(60), configName, MockCollector::sName));
...
void HostMonitorInputRunnerUnittest::TestPrepareForFirstCollect() const {
    { // case 1: between two points
        ...
        APSARA_TEST_EQUAL_FATAL(collectContext.GetMetricTime(), 15);
    ...
    { // case 3: at the edge of the report interval
        ...
        APSARA_TEST_EQUAL_FATAL(collectContext.GetMetricTime(), 30);
        APSARA_TEST_EQUAL_FATAL(collectContext.GetScheduleTime(), steadyClockNow + std::chrono::seconds(15 + 5));
    }
}
...
UNIT_TEST_CASE(HostMonitorInputRunnerUnittest, TestPrepareForFirstCollect);
